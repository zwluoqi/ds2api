...
It also mounts `./config.json` to `/data/config.json` and sets `DS2API_CONFIG_PATH=/data/config.json` by default, which avoids runtime persistence failures caused by read-only `/app`. Chat history, account stats, and runtime tokens are written under `/data/chat_history/`, `/data/account_stats/`, and `/data/account_tokens/`, backed by subdirectories under host `./data/`.
...
- `accounts`: managed DeepSeek accounts, supporting `email` or `mobile` login plus proxy/name/remark metadata, an optional `device_id`, and per-account `total_flash_limit` / `total_pro_limit` request limits.
