...
同时默认把 `./config.json` 挂载到容器 `/data/config.json`，并设置 `DS2API_CONFIG_PATH=/data/config.json`，用于避免 `/app` 只读导致运行时配置持久化失败。对话历史、账号统计和运行时 token 分别写入 `/data/chat_history/`、`/data/account_stats/`、`/data/account_tokens/`，对应宿主机 `./data/` 下的子目录。
