...
- **Persistent config**: the template mounts `/data` and sets `DS2API_CONFIG_PATH=/data/config.json`. After importing config in Admin UI, it will be written and persisted to this path. Account stats and runtime tokens should be placed under `/data/account_stats` and `/data/account_tokens` if you want them persisted on the same `/data` volume.
